...
const ADS_UF_DONT_REQUIRE_PASSWD = &H0020
...
Wscript.StdOut.Write """SamAccountName"",""Full Name"",""Created"",""Last Login"",""PasswordChanged"",""Password Never Expires"",""Password Not Required"",""Account is Disabled"",""Account Locked Out"",""No Kerberos Pre-Auth Required""" & vbCrlf
Do Until objRecordSet.EOF
 ...
 ELSE
  ...
  END IF
  IF objUser.GET("userAccountControl") AND ADS_UF_DONT_REQUIRE_PASSWD THEN
   Wscript.StdOut.Write """" & "TRUE" & ""","
  ELSE
   Wscript.StdOut.Write """" & "FALSE" & ""","
  END IF
  IF objUser.GET("userAccountControl") AND ACCOUNTDISABLE THEN
   Wscript.StdOut.Write """" & "TRUE" & ""","
  ELSE
   Wscript.StdOut.Write """" & "FALSE" & ""","
  END IF
  IF objUser.GET("userAccountControl") AND LOCKOUT THEN
   Wscript.StdOut.Write """" & "TRUE" & ""","
  ELSE
   Wscript.StdOut.Write """" & "FALSE" & ""","
  END IF
  IF objUser.GET("userAccountControl") AND DONT_REQ_PREAUTH THEN
   Wscript.StdOut.Write """" & "TRUE" & """"
  ELSE
   Wscript.StdOut.Write """" & "FALSE" & """"
  END IF
